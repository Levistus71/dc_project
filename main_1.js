...
function generateKMap2x4() {
    ...
    ['0', '1'].forEach((row, i) => {
        ...
        grayCode2.forEach((col, j) => {
            const index = i * 4 + grayToBinary(col); // Fix: Use Gray code to binary conversion for column index
            const value = truthTable[index] ? truthTable[index][variables.length] : '0';
            const cellClass = getCellClass(value);
...
function generateKMap4x4() {
    ...
    grayCode2.forEach((row, i) => {
        ...
        grayCode2.forEach((col, j) => {
            ...
            const index = rowBin * 4 + colBin; // Fix: Correctly calculate the index using row and column binary values
            const value = truthTable[index] ? truthTable[index][variables.length] : '0';
            const cellClass = getCellClass(value);
            html += `<td class="kmap-cell ${cellClass}">${value}</td>`;
        });
...
function drawLogicDiagram(terms) {
    ...
    if (!terms || terms.length === 0 || (terms.length === 1 && terms[0] === '0')) {
        container.innerHTML = '<div style="text-align:center; padding:20px; font-weight:bold; color:#555;">Output is 0</div>';
        return;
    }
    if (terms.length === 1 && terms[0] === '1') {
        container.innerHTML = '<div style="text-align:center; padding:20px; font-weight:bold; color:#555;">Output is 1</div>';
        return;
    }
